...
def getGloveEmbeddings():
    ...
    embs_npa = torch.from_numpy(embs_npa).float()
    v_to_glove = {}
    for e, v in zip(embs_npa, vocab_npa):
        v_to_glove[v] = e#torch.from_numpy(e.float())
...
def create_vocab(data, unk_threshold=UNK_THRESHOLD):
    ...
    # I'm changing this so that the glove tokens are the first ones
    with open('vocab_npa.npy','rb') as f:
        glove_vocab = np.load(f)
    in_glove, not_in_glove = [], []
    ...
    print('Have %s in glove, and %s not in glove' % (len(in_glove), len(not_in_glove)))

    vocab = in_glove + not_in_glove
    token_to_idx = {PAD: 0, UNK: 1}
    for token in vocab:
        token_to_idx[token] = len(token_to_idx)
    return token_to_idx, len(in_glove)
...
class SequenceClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, n_labels, n_rnn_layers, pad_idx, token_to_idx):
        super().__init__()

        self.pad_idx = pad_idx

        # Changed! Get glove embeddings
        v_to_glove= getGloveEmbeddings()
        np_matrix = np.empty((0,embedding_dim), float)
        total_pretrained = 0
        for token in token_to_idx:
            idx = token_to_idx[token]
            if token in v_to_glove:
                np_matrix = np.vstack([np_matrix, v_to_glove[token]])
                total_pretrained += 1
            else:
                np_matrix = np.vstack([np_matrix, np.random.rand(1, embedding_dim)])
        print('made a matrix of shape', np_matrix.shape)

        #print('making embedding with np matrix [0]')
        self.embedding = nn.Embedding.from_pretrained(torch.from_numpy(np.asarray(np_matrix)).float())
...
def main():
    ...
    token_to_idx, len_pretrained = create_vocab(train_data)
    ...
    model = SequenceClassifier(
        len(token_to_idx), EMBEDDING_DIM, HIDDEN_DIM, len(label_to_idx), N_RNN_LAYERS, pad_idx, token_to_idx
    )
